...
@interface  MyMoviePlayer : MPMoviePlayerViewController
...
- (void)resetOptionsBackgroundImages
{
    ...
    else
    {
        ...
        for (i=0; i<count; i++)
        {
            ...
            [optionButton setBackgroundImage:[UIImage imageNamed:nil] forState:UIControlStateHighlighted];
        }
    }
}
...
- (void)mediaTapped:(UITapGestureRecognizer *)inGesture
{
    ...
    else if (_currentQuestionType == eHAQuestionVideoType)
    {
        ...
        moviePlayer = [[MyMoviePlayer alloc] initWithContentURL:url];
...
- (void)sendTurn
{
    ...
    [[HATurnbasedMatchHelper sharedInstance] currentMatch];
    ...
    if (currentMatch.matchData.length > 0) //this is second and final turn
    {
        ...
        [currentMatch endMatchInTurnWithMatchData:data scores:@[] achievements:@[] completionHandler:^(NSError *error)
         {
             [HATurnbasedMatchHelper sharedInstance]._saveToLoseList = NO;
             if (error) //unable to end match
             {
                 ...
                 UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Oops" message:@"An error occured while ending match. Your Match will be resubmitted automatically when network is back." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
                 [alertView show];
    ...
    else
    {
        ...
        if (data.length <= currentMatch.matchDataMaximumSize)
        {
            ...
            [currentMatch endTurnWithNextParticipants:[NSArray arrayWithObject:nextParticipant] turnTimeout:GKTurnTimeoutNone matchData:data completionHandler:^(NSError *error) {
                if (error) {
                    UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Oops" message:@"An error occured while submitting your turn, please play again" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
                    [alertView show];
                    [self.navigationController popToRootViewControllerAnimated:YES];
                }
                else{
        ...
        {
            UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"" message:[NSString stringWithFormat:@"Multiplayer trasact data bytes are more please set set \"category_questions_max_limit\" property value for category : %@ before uploading app to store",[HAQuizDataManager sharedManager]._currentQuizCategoryName] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alertView show];
