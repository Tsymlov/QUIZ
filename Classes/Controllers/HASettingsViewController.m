...
- (void)viewDidLoad {
    ...
    if (([HASettings sharedManager]._isAdSupported == NO && [HASettings sharedManager]._isInAppPurchaseSupported)|| [HASettings sharedManager]._isAdsTurnedOff) {
...
- (void)buyAction:(id)sender
{
    if ([HASettings sharedManager]._isParentalGateEnabled)
    {
        ...
        NSArray* numbers = [NSArray arrayWithObjects:@"16",@"21",@"14",@"12",@"19",@"40",@"71",@"99",@"56",@"65", nil];
        int i = arc4random()%numbers.count;
        NSUInteger number = [[numbers objectAtIndex:i] intValue];
        _ansForParentalQuestion = number*2;
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Parental Gate" message:[NSString stringWithFormat:@"What is %lu X 2 = ?",(unsigned long)number] delegate:self cancelButtonTitle:@"Check" otherButtonTitles:@"Cancel", nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (self._restoreAlertView == alertView) {
        ...
        self._restoreAlertView = nil;
    }
    else{
        if (buttonIndex == 0) {
            if (_ansForParentalQuestion == [[[alertView textFieldAtIndex:0] text] integerValue]) {
