...
- (IBAction)getMoreCategories:(id)sender
{
    ...
    if (purchaseCategories.count == 0) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"You have already purchased all categories." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
        return;
    }
...
- (IBAction)worldScoreAction:(id)sender
{
    if ([HAUtilities isInternetConnectionAvailable]) {
        ...
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Network error" message:@"Please check your internet connection." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
    }
}
...
- (IBAction)settingsAction:(id)sender
{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        self.popoverViewController = [[UIPopoverController alloc] initWithContentViewController:controller];
        self.popoverViewController.backgroundColor = [UIColor clearColor];
        self.popoverViewController.popoverContentSize = CGSizeMake(320.0, 350.0);
        [self.popoverViewController presentPopoverFromBarButtonItem:self.navigationItem.leftBarButtonItem permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
    }
    else{
        UINavigationController* navController = [[UINavigationController alloc] initWithRootViewController:controller];
...
- (void)productsLoaded:(NSNotification *)nc
{
    if (self._showGetMoreCategories) {
        ...
        if (categories.count == 0) {
            UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"No categories available to buy." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alertView show];
        }
        else{
...
{
    UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"Failed to load products, please try again later" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
    [alertView show];
}
...
-(void)takeTurn:(GKTurnBasedMatch *)match
{
    ...
    if (isSameVersion) {
        ...
        if (isNewer == NO) {
            UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"UPDATE!" message:@"To accept this match, update your app to latest version" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:@"Update now",nil];
            [alertView show];
            return;
        }
    }
    ...
    if ([HASettings sharedManager]._isInAppPurchaseSupported) {
        if ([localCatgoryDict valueForKey:kProductIdentifier]) {
            if ([[NSUserDefaults standardUserDefaults] valueForKey:[localCatgoryDict valueForKey:kProductIdentifier]]) {
                ...
                UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Oops" message:[NSString stringWithFormat:@"This match contains premium questions. To accept this match you need to buy \"%@\" quiz from \"More categories\"",[localCatgoryDict objectForKey:kQuizCategoryName]] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
                [alertView show];
                return;
            }
        }
    ...
    if (error) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Oops" message:@"There is some game data issue with this match please play some other game" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
        return;
    }
...
- (void)layoutMatch:(GKTurnBasedMatch *)match
{
    ...
    if (statusString != nil) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Status" message:statusString delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
    }
}
