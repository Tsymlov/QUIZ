...
- (BOOL)validateSettings
{
    if (self._isAdSupported && (self._removeAdsProdcutIdentifier == nil || [self._removeAdsProdcutIdentifier isEqualToString:@""]))
    {
        ...
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Developer alert" message:@"You have enabled ads by setting value YES to property \"Enable Ads Support\" in \"Configuration.plist->Features Settings\" and not specified product identifier for removing ads through InApp purchase for property \"Remove Ads Product Identifier\". If you are not willing to support ads set NO to property \"Enable Ads Support\" in \"Configuration.plist->Features Settings\". Ads are turned off automatically." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
        return NO;
    }
    ...
    if (self._isGameCenterSupported) {
        ...
        for (NSDictionary* category in allcategories) {
            if ([category valueForKey:kLeaderboardID] == nil || [[category valueForKey:kLeaderboardID] isEqualToString:@""]) {
                UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Developer alert" message:[NSString stringWithFormat:@"You have enabled ads by setting value YES to property \"Enable Game Center\" in \"Configuration.plist->Features Settings\" and not specified \"leaderboardID\" for category \"%@\". If you are not willing to support game center please turn off from the category node from \"Quiz_Categories.plist or json\" file",[category objectForKey:kQuizCategoryName]] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
                [alertView show];
                return NO;
            }
        }
    ...
    {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Developer alert" message:@"Add iTune's link of this app for property at \"Configuration.plist->Features Settings->ApplicationiTunesLink\". You can find this link iTunes connect once you have created the app. This link will be shared on social n/ws along with score while sharing score." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alertView show];
        return NO;
    }
